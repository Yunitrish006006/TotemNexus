...
final class NexusMapScreen extends NexusOwnedScreen {
    ...
    NexusMapScreen(SpaceUnitMapPayload payload, boolean observer, Runnable stop){super(Component.translatable("message.totem.space_unit.map_title"), observer, stop);this.payload=payload;}
    @Override protected void init(){CURRENT=this;int panelWidth=panelWidth(),panelHeight=panelHeight(),x=(width-panelWidth)/2,y=(height-panelHeight)/2,buttonY=y+panelHeight-32;
        addRenderableWidget(Button.builder(Component.translatable("message.totem.space_unit.map_refresh"),ignored->refresh()).bounds(x+12,buttonY,62,18).build());
        favorite=addRenderableWidget(Button.builder(Component.translatable("message.totem.space_unit.map_favorite_add"),ignored->favorite()).bounds(x+82,buttonY,94,18).build());
        teleport=addRenderableWidget(Button.builder(Component.translatable("message.totem.space_unit.teleport_start"),ignored->teleport()).bounds(x+panelWidth-158,buttonY,70,18).build());
    ...
    private void buttons(){SpaceUnitMapPayload.Entry e=entry();if(teleport!=null)teleport.active=e!=null&&e.canTeleport();if(favorite!=null){favorite.active=e!=null;favorite.setMessage(Component.translatable(e!=null&&e.favorite()?"message.totem.space_unit.map_favorite_remove":"message.totem.space_unit.map_favorite_add"));}}
