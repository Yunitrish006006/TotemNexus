...
final class NexusFriendsScreen extends Screen {
    static NexusFriendsScreen CURRENT;
    private SpaceUnitFriendsPayload payload; private UUID selected; private Button remove;
    NexusFriendsScreen(SpaceUnitFriendsPayload payload) { super(Component.translatable("message.deadrecall.space_unit.friends_title")); this.payload=payload; }
    @Override protected void init(){CURRENT=this; int x=(width-330)/2,y=(height-238)/2;
        ...
        addRenderableWidget(Button.builder(Component.translatable("gui.done"), ignored->onClose()).bounds(x+256,y+208,62,18).build()); buttons(); if(payload==null)refresh(); }
    ...
    @Override public boolean mouseClicked(MouseButtonEvent event,boolean doubleClick){int y=(height-238)/2+42;for(SpaceUnitFriendsPayload.Entry e:entries()){if(event.y()>=y&&event.y()<y+24){selected=e.id();buttons();return true;}y+=28;}return super.mouseClicked(event,doubleClick);}
