...
final class NexusRegistrationPreviewScreen extends NexusOwnedScreen {
    ...
    NexusRegistrationPreviewScreen(SpaceUnitRegistrationPreviewPayload payload, boolean observer, Runnable stop) {
        super(Component.translatable("message.totem.space_unit.registration_gui_title"), observer, stop); this.payload = payload;
    }
    @Override protected void init() {
        int x=(width-WIDTH)/2,y=(height-HEIGHT)/2;
        addRenderableWidget(Button.builder(Component.translatable("message.totem.space_unit.registration_gui_confirm"), ignored -> confirm()).bounds(x+WIDTH-132,y+HEIGHT-28,58,18).build());
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor extractor,int mouseX,int mouseY,float partialTick){int x=(width-WIDTH)/2,y=(height-HEIGHT)/2;
        ...
        extractor.text(font,Component.translatable("message.totem.space_unit.registration_gui_position",payload.dimension(),payload.x(),payload.y(),payload.z()),x+12,y+29,0xFFB8C0C8);
        extractor.text(font,Component.translatable("message.totem.space_unit.registration_gui_structure",payload.tier(),payload.resonancePercent(),payload.completenessPercent(),payload.wearPercent()),x+12,y+47,0xFFE0E6EC);
        extractor.text(font,Component.translatable("message.totem.space_unit.registration_gui_timeout",payload.confirmSeconds()),x+12,y+65,0xFFFFD166);
        int hintY=y+84;for(var line:font.split(Component.translatable("message.totem.space_unit.registration_gui_hint"),WIDTH-24)){extractor.text(font,line,x+12,hintY,0xFF93A4B5);hintY+=10;} super.extractRenderState(extractor,mouseX,mouseY,partialTick);}
