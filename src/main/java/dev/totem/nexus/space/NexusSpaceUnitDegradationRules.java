...
public final class NexusSpaceUnitDegradationRules {
    ...
    private NexusSpaceUnitDegradationRules() {
    }
    ...
    private static void load(ResourceManager resourceManager) {
        ...
        for (Map.Entry<Identifier, Resource> entry : resources.entrySet()) {
            ...
            try (BufferedReader reader = entry.getValue().openAsReader()) {
                ...
                LOGGER.warn("無法載入 Space Unit 石碑退化規則 {}：{}", resourceId, exception.getMessage());
            }
        }

        degradationTargets = Map.copyOf(loaded);
        LOGGER.info("已載入 {} 個 Space Unit 石碑退化規則", degradationTargets.size());
    }
    ...
    private static void parseRules(Identifier resourceId, JsonObject json, Map<Block, Block> output) {
        ...
        if (rules.isEmpty()) {
            LOGGER.warn("Space Unit 石碑退化規則 {} 沒有任何 rule", resourceId);
        }
    }
