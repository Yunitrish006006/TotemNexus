...
public class NexusSpaceDiscoverySavedData extends SavedData {
    ...
    /** Removes a purged unit from every player's discovery and favorite state. */
    public boolean removeUnitReferences(UUID unitId) {
        boolean changed = false;
        for (Set<UUID> units : this.discoveredByPlayer.values()) {
            changed |= units.remove(unitId);
        }
        for (Set<UUID> favorites : this.favoritesByPlayer.values()) {
            changed |= favorites.remove(unitId);
        }
        this.discoveredByPlayer.entrySet().removeIf(entry -> entry.getValue().isEmpty());
        this.favoritesByPlayer.entrySet().removeIf(entry -> entry.getValue().isEmpty());
        if (changed) {
            setDirty();
        }
        return changed;
