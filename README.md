...
目前模組版本為 **0.3.8**，需要 TotemCore **0.7.13 以上、0.8.0 以下**。
...
在來源磁石附近開啟 Space Unit 地圖並切到「材料」，可按「顯示傳送陣」取得
一次新的 Server 權威掃描。關閉畫面後，實際計入的材料會以可穿牆框線顯示
30 秒：青色為一般計入方塊、金色為會繼續延伸掃描的材料、紫色為來源磁石。
再次按下按鈕可提前隱藏。此功能只適用於目前地圖的本地來源，不會顯示遠端
目的地、未載入區塊或未被延伸路徑連到的材料。
